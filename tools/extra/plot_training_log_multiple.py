...
# This function is modified to support multiple chart_types in a single plot
def is_valid_chart_type(chart_types):
    flag = True
    for chart_type in chart_types:
        if (chart_type < 0 or chart_type >= len(get_supported_chart_types())):
            flag = False
            return flag
    return flag 
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        # Support multiple type of ploting
        chart_types = [int(s) for s in list(sys.argv[1])]

        if not is_valid_chart_type(chart_types):
            print_help()
