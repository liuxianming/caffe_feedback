...
namespace caffe {
  ...
  */
  ...
  /*
   * RELU+ layer, which implements the feedback / selectivity properties
   * The RELU+ layer is used upon the RELU layer, 
   * to select the outputs of RELU neurons
   */
  template<typename Dtype>
  ...
  public:   
    explicit ReLUPlusLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param){}
    //override the setup function to initialize activation
    virtual void SetUp(const vector<Blob<Dtype>*>& bottom, 
		       vector<Blob<Dtype>*>* top);
  protected: 
    virtual Dtype Forward_cpu(const vector<Blob<Dtype>*>& bottom,
