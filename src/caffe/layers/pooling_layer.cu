...
namespace caffe {
  ...
  __global__ void MaxPoolForward(const int nthreads, const Dtype* bottom_data,
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
      ...
      //A potential error: offset of the bottom_data
      const Dtype* bottom_data_m = bottom_data + 
	(n * channels + c) * height * width;
      int maxidx = 0;
      for (int h = hstart; h < hend; ++h) {
	for (int w = wstart; w < wend; ++w) {
          if(maxval < bottom_data_m[h*width + w]){
	    maxidx = h*width + w;
	    maxval = bottom_data_m[maxidx];
          }
	}
      }
      top_data[index] = maxval;
      mask_data[index] = static_cast<Dtype> (maxidx);
    }
  }

  //ToDo: modify other pooling method if current modification works
