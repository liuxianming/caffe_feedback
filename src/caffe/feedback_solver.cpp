...
FeedbackSolver<Dtype>::FeedbackSolver(const SolverParameter& param)
    : net_(), test_net_() {
  Init(param);
}
...
template <typename Dtype>
void FeedbackSolver<Dtype>::Init(const SolverParameter& param) {
  param_ = param;
  ...
  net_.reset(new FeedbackNet<Dtype>(param_.train_net()));
  if (param_.has_test_net()) {
    LOG(INFO) << "Creating testing net.";
    test_net_.reset(new FeedbackNet<Dtype>(param_.test_net()));
...
template <typename Dtype>
void FeedbackSolver<Dtype>::Test() {
...
template <typename Dtype>
void FeedbackSolver<Dtype>::Snapshot() {
...
template <typename Dtype>
void FeedbackSolver<Dtype>::Restore(const char* state_file) {
  SolverState state;
  NetParameter net_param;
...
template <typename Dtype>
Dtype SGDFeedbackSolver<Dtype>::GetLearningRate() {
  Dtype rate;
...
template <typename Dtype>
void SGDFeedbackSolver<Dtype>::PreSolve() {
...
template <typename Dtype>
void SGDFeedbackSolver<Dtype>::SnapshotSolverState(SolverState* state) {
  state->clear_history();
...
template <typename Dtype>
void SGDFeedbackSolver<Dtype>::RestoreSolverState(const SolverState& state) {
...
INSTANTIATE_CLASS(FeedbackSolver);
INSTANTIATE_CLASS(SGDFeedbackSolver);
